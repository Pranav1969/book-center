...
export default function StockManagement() {
  ...
  async function fetchStockData() {
    setLoading(true);
    // Added 'publication' to the select query
  ...
  const filteredBooks = books
    .filter(book => {
      ...
      // Searches in both Title and Publication
      const matchesSearch = 
    ...
    .sort((a, b) => {
      if (sortBy === "stock_count" || sortBy === "price") return a[sortBy] - b[sortBy];
  ...
  // ... (handleUpdateStock and handleDeleteBook functions remain the same) ...
  ...
  return (
    <div className="p-8 bg-stone-50 min-h-screen text-stone-900 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* TABLE BODY (Modified to show Publication) */}
                  ...
                  <td className="p-5 w-48">
                    <div className="w-full bg-stone-100 h-1 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min((book.stock_count / 25) * 100, 100)}%` }}
                      ></div>
                    </div>
                  </td>
                  ...
                  <td className="p-5 text-right">
                    <div className="flex justify-end items-center gap-3">
                      <div className="flex bg-stone-50 rounded-full border border-stone-200 px-1">
                        <button onClick={() => handleUpdateStock(book.id, book.stock_count, -1)} className="w-7 h-7 flex items-center justify-center hover:text-red-500 transition-colors">−</button>
                        <button onClick={() => handleUpdateStock(book.id, book.stock_count, 1)} className="w-7 h-7 flex items-center justify-center hover:text-teal-600 transition-colors">+</button>
                      </div>
                      <button onClick={() => handleDeleteBook(book.id, book.title)} className="text-[9px] font-black uppercase text-stone-300 hover:text-red-600 transition-colors">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
