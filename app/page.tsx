...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (session?.user) {
        ...
        // CHECK: Does this user have a completed profile?
        ...
        // If logged in but no profile name, redirect to setup
  ...
  // 🆕 Function to handle "Buy" or "Protected" actions
        ...
        >
          <div className="group relative bg-[#0d071a] rounded-3xl overflow-hidden flex flex-col md:flex-row items-center border border-white/5 shadow-2xl">
            <div className="p-8 md:p-20 md:w-1/2 space-y-6">
              ...
              <button 
                onClick={(e) => handleProtectedAction(e, campaign.target_url || "/catalog")}
                className="inline-flex items-center gap-3 bg-white text-black px-8 py-3.5 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-purple-600 hover:text-white transition-all"
              >
      ...
      {selectedCategory === "All" && bestSellers.length > 0 && !searchQuery && (
        <section className="py-12 md:py-24">
          ...
          <div className="max-w-7xl mx-auto md:px-4">
            <div className="flex md:grid md:grid-cols-4 lg:grid-cols-5 overflow-x-auto md:overflow-x-visible snap-x snap-mandatory no-scrollbar gap-6 md:gap-10 px-4 md:px-0">
              {bestSellers.map((book) => (
                <div key={book.id} className="relative group flex-shrink-0 w-[70vw] md:w-auto snap-center md:snap-align-none cursor-pointer">
                  ...
                  <div onClick={(e) => handleProtectedAction(e, `/book/${book.id}`)}>
                    <BookCard book={book} />
                  </div>
                </div>
              ))}
      ...
      <footer className="bg-[#020005] pt-20 pb-10 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="flex flex-col gap-4 text-[10px] font-bold uppercase tracking-widest text-gray-500">
            ...
            <Link href="/#bestsellers" className="hover:text-white transition-colors">Best Sellers</Link>
